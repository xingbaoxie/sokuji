...
# original path: genie_tts/G2P/Chinese/ToneSandhi.py. Local modifications are
...
import jieba  # SOKUJI: jieba_fast -> pure-python jieba (identical API)
...
class ToneSandhi:
    ...
    # SOKUJI: vowel-less syllabic nasals (嗯/呣) produce an empty finals string
    # from lazy_pinyin (e.g. lazy_pinyin("嗯", style=Style.FINALS_TONE3) ==
    # ['']). _merge_continuous_three_tones_2 used to index [-1][-1] straight
    # into that string/list and raise IndexError. These helpers enforce the
    # invariant "never apply [-1] to an empty string/list" before checking
    # tone; a word with an empty or missing final is simply not tone-three.
    @staticmethod
    def _last_final_is_tone_three(finals: List[str]) -> bool:
        return bool(finals) and bool(finals[-1]) and finals[-1][-1] == "3"

    @staticmethod
    def _first_final_is_tone_three(finals: List[str]) -> bool:
        return bool(finals) and bool(finals[0]) and finals[0][-1] == "3"
    ...
    def _merge_continuous_three_tones_2(self, seg: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
        ...
        for i, (word, pos) in enumerate(seg):
            if (
                    i - 1 >= 0
                    # SOKUJI: guarded against empty finals (see helper docstring above) —
                    # was "sub_finals_list[i - 1][-1][-1] == '3' and sub_finals_list[i][0][-1] == '3'"
                    and self._last_final_is_tone_three(sub_finals_list[i - 1])
                    and self._first_final_is_tone_three(sub_finals_list[i])
                    and not merge_last[i - 1]
            ):
                # if the last word is reduplication, not merge, because reduplication need to be _neural_sandhi
                if not self._is_reduplication(seg[i - 1][0]) and len(seg[i - 1][0]) + len(seg[i][0]) <= 3:
