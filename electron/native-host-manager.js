...
class NativeHostManager {
  constructor() {
    ...
    this.port = null;
    this._starting = null;
    // This token is held only by Electron Main and inherited by the localhost
    // sidecar. It is intentionally absent from every renderer IPC result.
    this.recordingToken = crypto.randomBytes(32).toString('base64url');
  }
  ...
  start() {
    ...
    this._starting = new Promise((resolve, reject) => {
      ...
      // Native-host unit tests and developer overrides deliberately run without
      // Electron's binary present. Only load Electron when a userData lookup is
      // genuinely needed; an explicit HF_HOME + Python override needs neither.
      const needsElectronApp = !process.env.HF_HOME || (!envOverride && !process.env.SOKUJI_USERDATA);
      const app = needsElectronApp ? require('electron').app : null;
      ...
      const env = { ...process.env, HF_HOME: hfHome, SOKUJI_RECORDING_TOKEN: this.recordingToken };
  ...
  recordingAuth() { return { token: this.recordingToken }; }
