...
/**
 * The source Electron app is used primarily to exercise the recording POC.
 * Keep its six-step, general-product onboarding out of that loop while still
 * leaving the packaged app and test runs unchanged.  This deliberately stays
 * in memory: it does not create a fake user configuration or overwrite a
 * real setup record.  Set VITE_SOKUJI_SKIP_SETUP=false to show the wizard.
 */
function skipFirstRunSetupInDevelopment(): boolean {
  return import.meta.env.MODE === 'development' && import.meta.env.VITE_SOKUJI_SKIP_SETUP !== 'false';
}

function developmentSetupRecord(): SetupRecord {
  return {
    version: SETUP_VERSION,
    scenario: null,
    providerPath: null,
    provider: 'development-skip',
  ...
  subscribeWithSelector((set, get) => ({
    ...
    hydrate: async () => {
      ...
      try {
        ...
        if (skipFirstRunSetupInDevelopment()) {
          set({ setup: developmentSetupRecord(), tour, loaded: true });
          return;
        }
