...
// Keep the Chinese default and English available synchronously; all other
// interface languages remain lazy-loaded.
...
const DEV_LANGUAGE_SEED_KEY = 'sokuji.dev.interface-language-seeded';

/**
 * Earlier development launches let Electron's English locale populate
 * `i18nextLng`. Seed Chinese once for that isolated development profile, then
 * get out of the way: any choice made in Settings wins on later launches.
 */
function developmentInitialLanguage(): string | undefined {
  if (import.meta.env.MODE !== 'development') return undefined;
  try {
    if (localStorage.getItem(DEV_LANGUAGE_SEED_KEY)) return undefined;
    localStorage.setItem(DEV_LANGUAGE_SEED_KEY, '1');
    return 'zh_CN';
  } catch {
    return undefined;
  }
...
i18n
  ...
  .init({
    resources,
    lng: developmentInitialLanguage(),
    // A fresh install should open in Chinese. A language explicitly selected
    // in Settings remains in localStorage and always takes precedence.
    fallbackLng: 'zh_CN',
    ...
    detection: {
      // Do not let Electron's English process locale override the product
      // default on a clean profile. Users can still select any supported
      // interface language and that choice is persisted here.
      order: ['localStorage'],
      caches: ['localStorage'],
    },
