...
const RecordingSettingsSection: React.FC = () => {
  ...
  return <div className="recording-settings" aria-label={t('recording.title')}>
    ...
    <section className="settings-section"><div className="recording-settings__feature-header"><h2>{t('recording.generateTranslation')}</h2><ToggleSwitch checked={config.translation.enabled} onChange={() => update({ translation: { ...config.translation, enabled: !config.translation.enabled } })} label={config.translation.enabled ? t('common.on') : t('common.off')} /></div>{config.translation.enabled && <><div className="recording-settings__form-grid"><label>{t('recording.targetLanguage')}<select className={SELECT_CLASS_NAME} value={config.targetLanguage} onChange={(event) => update({ targetLanguage: event.target.value })}>{LANGUAGES.map((language) => <option value={language} key={language}>{getLanguageOption(language).name}</option>)}</select></label><label>{t('recording.providerLabel')}<select className={SELECT_CLASS_NAME} value={config.translation.providerId} onChange={(event) => selectProvider('translation', event.target.value as RecordingProviderId)}>{PROVIDERS.map((id) => <option key={id} value={id}>{label(id)}</option>)}</select></label>{config.translation.providerId === 'private-runtime' && <label>{t('recording.model')}<select className={SELECT_CLASS_NAME} value={config.translation.modelId} disabled={!textStatus('translation')?.models?.length} onChange={(event) => update({ translation: { ...config.translation, modelId: event.target.value } })}><option value="">{textStatus('translation')?.state === 'disabled' ? t('recording.modelUnavailable') : t('recording.chooseModel')}</option>{(textStatus('translation')?.models || []).map((model) => <option key={model.id} value={model.id}>{model.id}</option>)}</select></label>}</div>{config.translation.providerId === 'private-runtime' ? <PrivateCard profileId="translation.private" /> : <CloudCard profileId="translation.aliyun" />}</>}</section>
    <section className="settings-section"><div className="recording-settings__feature-header"><h2>{t('recording.generateSummary')}</h2><ToggleSwitch checked={config.summary.enabled} onChange={() => update({ summary: { ...config.summary, enabled: !config.summary.enabled, inputMode: !config.summary.enabled && !config.translation.enabled ? 'source' : config.summary.inputMode } })} label={config.summary.enabled ? t('common.on') : t('common.off')} /></div>{config.summary.enabled && <><div className="recording-settings__form-grid"><label>{t('recording.providerLabel')}<select className={SELECT_CLASS_NAME} value={config.summary.providerId} onChange={(event) => selectProvider('summary', event.target.value as RecordingProviderId)}>{PROVIDERS.map((id) => <option key={id} value={id}>{label(id)}</option>)}</select></label>{config.summary.providerId === 'private-runtime' && <label>{t('recording.model')}<select className={SELECT_CLASS_NAME} value={config.summary.modelId} disabled={!textStatus('summary')?.models?.length} onChange={(event) => update({ summary: { ...config.summary, modelId: event.target.value } })}><option value="">{textStatus('summary')?.state === 'disabled' ? t('recording.modelUnavailable') : t('recording.chooseModel')}</option>{(textStatus('summary')?.models || []).map((model) => <option key={model.id} value={model.id}>{model.id}</option>)}</select></label>}<label>{t('recording.summaryInput')}<select className={SELECT_CLASS_NAME} value={config.summary.inputMode} onChange={(event) => update({ summary: { ...config.summary, inputMode: event.target.value as RecordingJobConfig['summary']['inputMode'] } })}><option value="source">{t('recording.summarySource')}</option>{config.translation.enabled && <><option value="translated">{t('recording.summaryTranslated')}</option><option value="bilingual">{t('recording.summaryBilingual')}</option></>}</select></label><label>{t('recording.reportLanguage', { defaultValue: '报告语言' })}<select className={SELECT_CLASS_NAME} value={config.summary.reportLanguage} onChange={(event) => update({ summary: { ...config.summary, reportLanguage: event.target.value as RecordingJobConfig['summary']['reportLanguage'] } })}><option value="auto">{t('recording.reportLanguageAuto', { defaultValue: '跟随界面' })}</option><option value="zh">简体中文</option><option value="en">English</option><option value="ja">日本語</option></select></label><label>{t('recording.summaryTemplate', { defaultValue: '总结模板' })}<select className={SELECT_CLASS_NAME} value={config.summary.templateId} onChange={(event) => update({ summary: { ...config.summary, templateId: event.target.value } })}><option value="general-meeting">{t('recording.summaryTemplateGeneralMeeting', { defaultValue: '通用会议总结' })}</option></select></label></div>{config.summary.providerId === 'private-runtime' ? <PrivateCard profileId="summary.private" /> : <CloudCard profileId="summary.aliyun" />}</>}</section>
